...
fn check_jsons(name: &str, es_json: Value, rs_json: Value) {
    ...
    let es_name = format!("{}.es.json", name);
    let rs_name = format!("{}.rs.json", name);
    if ::std::env::vars().any(|a| a.0 == "RES_WRITE_JSON" 
        && a.1 == "1") {

        let esf = ::std::fs::File::create(&es_name).expect("failed to create es json file");
        let rsf = ::std::fs::File::create(&rs_name).expect("failed to create rs json file");
        to_writer_pretty(esf, &es_json).expect("failed to write es json");
        to_writer_pretty(rsf, &rs_json).expect("failed to write rs json");
    }
    panic!("json doesn't match if env var RES_WRITE_JSON=1, check files {} & {}", es_name, rs_name);
}
#[test]
fn test1() {
